...
#define SCTLR_EOE_LITTLE_ENDIAN                                                \
  (0 << 24) // Endianness of explicit data access at EL0.
...
// Exception Endianness: Lower Endian, Instruction/Data Cache Disabled, MMU
// Disabled.
...
#define HCR_RW                                                                 \
  (1 << 31) // 0:aarch32, 1:aarch64, Execution state control for lower Exception
            // levels:
...
#define SCR_RW                                                                 \
  (1 << 10) // 0:aarch32, 1:next lower EL is aarch64, Execution state control
            // for lower Exception levels.
#define SCR_NS                                                                 \
  (1 << 0) // 0: EL0,EL1 are in secure state, 1: all EL lower than EL3 are in
           // non secure state, Non-secure bit.

// Execution state for all lower ELs to be aarch64, all ELs lower than EL3 are
// in non secure state
...
#define SPSR_EL1h                                                              \
  (5 << 0) // AArch64 Exception level and selected Stack Pointer.

// Mask Debug Exception, SError Interrupt, IRQ interrupt, FIQ interrupt
#define SPSR_VALUE (SPSR_MASK_ALL | SPSR_EL1h)

// ***************************************
// ESR_EL1, Exception Syndrome Register (EL1). Page 1899 of
// AArch64-Reference-Manual.
...
// EC, [31:26] Exception Class. Indicates the reason for the exception that this
// register holds information about. ISS, [24:0] Syndrome information.
