...
var scanRecord = make(map[ethernet.MAC]*mistbssInfo)
...
func processBeacon(no int, packet gopacket.Packet) {
	...
	if ok {
		...
		beaconRecord.rssi_low = wifi.RSSI(rt.DBMAntennaSignal)
		beaconRecord.rssi_high = wifi.RSSI(rt.DBMAntennaSignal)
		beaconRecord.channel = channel
		beaconRecord.ie_channelset.Primary = channel
		band := channel.Band()
		...
		if beaconRecord.ie_channelset.Primary == beaconRecord.channel {
			...
			sc, ok := scanRecord[beaconRecord.bssid]
			if !ok {
				sc = &mistbssInfo{}
				sc.ssid = beaconRecord.ssid
				sc.bssid = beaconRecord.bssid
				sc.channel = beaconRecord.channel
				sc.ie_channelset = beaconRecord.ie_channelset
				sc.rssi_low = beaconRecord.rssi_low
				sc.rssi_high = beaconRecord.rssi_high
				scanRecord[sc.bssid] = sc
			} else {
				if beaconRecord.rssi_low < sc.rssi_low {
					sc.rssi_low = beaconRecord.rssi_low
				}
				if beaconRecord.rssi_high > sc.rssi_high {
					sc.rssi_high = beaconRecord.rssi_high
				}
			}
...
func main() {
	...
	fmt.Printf("\n\n ScanRecord\n\n")
	i = 0
	for _, sc := range scanRecord {
		i = i + 1
		fmt.Printf("%d] BSSID %v SSID %v channel %v ie_channelset %v rssi_high %v rssi_low %v\n", i, sc.bssid, sc.ssid, sc.channel, sc.ie_channelset, sc.rssi_high, sc.rssi_low)
	}
}
