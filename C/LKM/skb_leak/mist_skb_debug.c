...
int config_show(struct seq_file *m, void *arg) {
	...
	seq_printf(m, "CaptureType(T)=%d(%s)\n", mist_cap_type, ctype_itoa[mist_cap_type]);
	seq_printf(m, "SKBidx(I)=%d\n", mist_skb_idx);
	seq_printf(m, "SKBCntr(i)=%d\n", mist_skb_Cntr);
	return 0;
}

ssize_t config_wr(struct file *filp, const char __user * buf, size_t count, loff_t * loff) {
...
void update_counters(struct sk_buff *skb) {
	...
	if (module >= 0 && module < MIST_MOD_LAST && alloc_cap_point >= 0 && alloc_cap_point < MIST_CAP_LAST) {
		...
		dprintf("NCC[%s] module=%d alloc_cap_point=%d\n", __func__, module, alloc_cap_point);
	}
...
void mist_skb_dump(struct seq_file *m, struct sk_buff *skb, int *skbCount) {
	...
	if (logSKB) {
		seq_printf(m, "\n%d] skb=%px len=%d module=%s alloc_cap_point=%d last_seen=%s\n", *skbCount, skb, skb->len, module_itoa[module], alloc_cap_point, module_itoa[module_last_seen]);
	}

	if (logSKB && mist_dump_cmd == 2 && skb && (mist_skb_Cntr == -1 || ((--mist_skb_Cntr) > 0))) {
		...
		for (j = 0; j < objsPerSlab; j++) {
			...
			if (skb->mist_skb_dbg_magic == MIST_SKB_DBG_MAGIC) {
				...
				if (mist_skb_dbg_log_level > 1) {
					printk("pfn=0x%lx mist_alloced=0x%x alloced_mist_module=0x%x mist_alloced_cap_point=0x%x last_seen_module=0x%x\n", virt_to_pfn(skb), skb->mist_alloced, skb->alloced_mist_module, skb->mist_alloced_cap_point, skb->last_seen_module);
				}
				if (m) {
